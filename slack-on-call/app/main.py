...
DEV_GROUP = os.environ.get("DEV_GROUP_NAME")
SRE_GROUP = os.environ.get("SRE_GROUP_NAME")
...
def notify_slack(dev: list[str] = None, sre: list[str] = None) -> None:
    ...
    if not json.loads(resp)["ok"]:
        ...
        error_slack(
            header="Failed to notify about new schedule", message=json.loads(resp)
        )
        sys.exit(1)
...
def get_user_display_name(user_id: str = None) -> str:
    ...
    if not resp["ok"]:
        ...
        error_slack(
            header="Failed to get get_user_display_name", message=json.loads(resp)
        )
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        error_slack(
            header="Failed to Get usergroups from Slack", message=json.dumps(ex.args[0])
        )
        sys.exit(1)
    ...
    try:
        ...
        print(json.dumps(ex.args[0], indent=2), flush=True)
        error_slack(
            header="Failed to get user users in group from Slack",
            message=json.dumps(ex.args[0]),
        )
        sys.exit(1)
    except Exception as ex:
